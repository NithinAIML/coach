...
//   /* Wizard state, Step 1–4 (unchanged functionality) */
...
//   const canContinue = activeStep === 1 ? isStep1Valid : activeStep === 3 ? processed && !processing : true;
//   function goNext() {
...
//   }
//   function goPrev() { if (activeStep === 1) return; setActiveStep(s => Math.max(1, s - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }
...
//                     <button type="button" className={`${styles.btn} ${styles.btnGhost}`} onClick={goPrev} disabled={activeStep === 1 || processing}>Previous</button>
//                     <button type="button" className={`${styles.btn} ${styles.btnPrimary}`} onClick={goNext} disabled={!canContinue}>{activeStep === 3 && processing ? 'Processing…' : activeStep === TOTAL_STEPS ? 'Finish' : 'Continue'}</button>
//                   </div>
//                 </div>
//               </div>
//             </>
...
import React, { useMemo, useRef, useState } from 'react';
import styles from './home.module.scss';
import { uploadFileToS3, putS3 } from 'src/utils/s3'; // <-- NEW: use our S3 helpers
...
type IconProps = { size?: number };
const Icon = {
  Plus: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>),
  ChartBars: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M4 19h16M7 17V9M12 17V5M17 17v-6"/></svg>),
  Database: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M4 7c0 1.7 3.6 3 8 3s8-1.3 8-3-3.6-3-8-3-8 1.3-8 3zm16 5c0 1.7-3.6 3-8 3s-8-1.3-8-3m16 5c0 1.7-3.6 3-8 3s-8-1.3-8-3V7"/></svg>),
  Calendar: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M7 3v3M17 3v3M4 9h16M6 5h12a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2z"/></svg>),
  TrendUp: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M3 17l6-6 4 4 7-7M15 8h5v5"/></svg>),
  Cog: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M12 8a4 4 0 1 1 0 8 4 4 0 0 1 0-8z"/><path d="M12 2v3M12 19v3M4.6 5.6l2.1 2.1M17.3 18.3l2.1 2.1M2 12h3M19 12h3M4.6 18.4l2.1-2.1M17.3 5.7l2.1-2.1"/></svg>),
  File: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V9z"/><path d="M14 3v6h6"/></svg>),
  Upload: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M12 16V6"/><path d="M8 10l4-4 4 4"/><path d="M5 20h14"/></svg>),
  Globe: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18z"/><path d="M2.5 12h19"/><path d="M12 2.5c3 3.5 3 15.5 0 19"/><path d="M12 2.5c-3 3.5-3 15.5 0 19"/></svg>),
  Cloud: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M20 17a4 4 0 0 0-3.5-3.96A5 5 0 0 0 7 12a4 4 0 0 0-1 7.87h12A4 4 0 0 0 20 17z"/></svg>),
  Confluence: ({ size = 18 }: IconProps) => (<svg className={styles.icon} style={{ width: size, height: size }} viewBox="0 0 24 24"><path d="M6 4h9l3 3v13a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z"/><path d="M15 4v4h4"/><path d="M8 11h8M8 15h8"/></svg>),
  Check: ({ size = 16 }: IconProps) => (<svg viewBox="0 0 24 24" style={{ width: size, height: size }}><path d="M20 6L9 17l-5-5" stroke="currentColor" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>),
  Key: ({ size = 16 }: IconProps) => (<svg viewBox="0 0 24 24" style={{ width: size, height: size }}><path d="M21 7a5 5 0 1 1-9.8 1H3v4h5v4h4v-4h1.2A5 5 0 0 1 21 7z" fill="currentColor"/></svg>),
  Sun: ({ size = 16 }: IconProps) => (<svg viewBox="0 0 24 24" style={{ width: size, height: size }}><circle cx="12" cy="12" r="4" fill="currentColor"/><path d="M12 2v3M12 19v3M4.6 4.6l2.1 2.1M17.3 17.3l2.1 2.1M2 12h3M19 12h3M4.6 19.4l2.1-2.1M17.3 6.7l2.1-2.1" stroke="currentColor" fill="none"/></svg>),
  Moon: ({ size = 16 }: IconProps) => (<svg viewBox="0 0 24 24" style={{ width: size, height: size }}><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z" fill="currentColor"/></svg>),
};
...
const SelfServicePortal: React.FC = () => {
  ...
  // ---------- File Upload state ----------
  type UploadItem = { file: File; name: string; key?: string; status: 'pending' | 'uploading' | 'done' | 'error'; error?: string };
  const [fileUpload, setFileUpload] = useState<{
    name: string; description: string; autoRefresh: boolean; frequency: 'Daily'|'Weekly'|'Monthly'; time: string; files: File[];
  }>({ name: '', description: '', autoRefresh: true, frequency: 'Weekly', time: '09:00', files: [] });
  const [uploads, setUploads] = useState<UploadItem[]>([]);
  const [uploadPrefix] = useState<string>('knowledge/uploads'); // S3 folder prefix, adjust if you want
  ...
  function removeSource(kind: string) {
    ...
    if (selectedSource === kind) setSelectedSource(null);
    if (kind === 'Confluence') setConfluence({ name: '', url: '', description: '', autoRefresh: true, frequency: 'Weekly', time: '09:00' });
    if (kind === 'File Upload') { setFileUpload({ name: '', description: '', autoRefresh: true, frequency: 'Weekly', time: '09:00', files: [] }); setUploads([]); }
  }

  // ---------- S3 upload logic (as-is binary) ----------
  async function pushSingleFileToS3(file: File) {
    const item: UploadItem = { file, name: file.name, status: 'uploading' };
    setUploads(prev => [...prev, item]);

    try {
      const key = `${uploadPrefix}/${Date.now()}-${file.name}`; // unique-ish key; adjust as needed
      const res = await uploadFileToS3(file, { key });
      setUploads(prev => prev.map(u => (u === item ? { ...u, status: 'done', key: res.key } : u)));
    } catch (err: any) {
      setUploads(prev => prev.map(u => (u === item ? { ...u, status: 'error', error: err?.message || 'Upload failed' } : u)));
    }
  }
  ...
  const onDrop: React.DragEventHandler<HTMLDivElement> = async (e) => {
    ...
    if (!f.length) return;
    // keep local list (unchanged behavior)
    setFileUpload(p => ({ ...p, files: [...p.files, ...f] }));
    // upload each
    for (const file of f) await pushSingleFileToS3(file);
  };
  const onDragOver: React.DragEventHandler<HTMLDivElement> = (e) => e.preventDefault();
  ...
  const onFilePick: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    setFileUpload(p => ({ ...p, files: [...p.files, ...f] }));
    for (const file of f) await pushSingleFileToS3(file);
    // clear the input so the same file can be chosen again if desired
  ...
  const canContinue = activeStep === 1 ? Object.keys(errors).length === 0 : activeStep === 3 ? processed && !processing : true;
  ...
  function goNext() {
    ...
    if (activeStep === 1) {
      setTouched({ teamName: true, department: true, domain: true, contactEmail: true, description: true });
      if (!isStep1Valid) return;
      // Optional: save registration JSON to S3 (kept off by default)
      // putS3(form, { key: `registrations/${form.teamName.replace(/\s+/g,'_')}-${Date.now()}.json` }).catch(console.error);
    }
    setActiveStep(s => Math.min(TOTAL_STEPS, s + 1));
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }
  ...
  // ----- Views for other tabs (unchanged visual/behavior from your last good build) -----
  const DashboardView = (
    <div style={{ display: 'grid', gap: 16 }}>
      <Card pad={16}>
        <div style={{ display: 'grid', gap: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', alignItems: 'center', padding: '14px 16px', border: '1px solid var(--line)', borderRadius: 10, background: '#fff' }}>
            <div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6 }}>Total queries</div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>1,247</div>
              <div style={{ fontSize: 12, color: '#16a34a', marginTop: 6 }}>↑ 12% from last week</div>
            </div>
            <div style={{ width: 28, height: 28, borderRadius: 8, display: 'grid', placeItems: 'center', color: 'var(--primary)', background: '#f3f4f6' }}><Icon.TrendUp size={14} /></div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', alignItems: 'center', padding: '14px 16px', border: '1px solid var(--line)', borderRadius: 10, background: '#fff' }}>
            <div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6 }}>Avg response time</div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>0.8s</div>
              <div style={{ fontSize: 12, color: '#ef4444', marginTop: 6 }}>↓ 0.2s from last week</div>
            </div>
            <div style={{ width: 28, height: 28, borderRadius: 8, display: 'grid', placeItems: 'center', color: 'var(--primary)', background: '#f3f4f6' }}>⚡</div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', alignItems: 'center', padding: '14px 16px', border: '1px solid var(--line)', borderRadius: 10, background: '#fff' }}>
            <div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6 }}>Satisfaction rate</div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>94%</div>
              <div style={{ fontSize: 12, color: '#16a34a', marginTop: 6 }}>↑ 3% from last week</div>
            </div>
            <div style={{ width: 28, height: 28, borderRadius: 8, display: 'grid', placeItems: 'center', background: '#eef2ff', color: 'var(--primary)' }}>★</div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', alignItems: 'center', padding: '14px 16px', border: '1px solid var(--line)', borderRadius: 10, background: '#fff' }}>
            <div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6 }}>Knowledge gaps</div>
  ...
  // Auto-refresh, Analytics, Settings… (omitted here for brevity; keep your last working sections)
    ...
    <>
      ...
      <main className={styles.page}>
        <div className={styles.container} style={{ minWidth: 0 }}>
          {section === 'dashboard' && DashboardView}
          {section === 'knowledge'  && KnowledgeSourcesView}
          {/* auto / analytics / settings views are same as your last accepted version */}
            ...
            <>
              ...
              <div className={styles.cardWrap}>
                <div className={styles.card}>
                  ...
                  {/* Step 2 (only File Upload segment modified to upload-as-is) */}
                  {activeStep === 2 && (
                    <>
                      <div className={styles.sourcesGrid} role="group" aria-label="Add knowledge source">
                        <button type="button" className={`${styles.sourceBtn} ${selectedSource === 'Confluence' ? styles.sourceBtnActive : ''}`} onClick={() => addSource('Confluence')} aria-pressed={selectedSource === 'Confluence'}>
                      ...
                      {selectedSource === 'File Upload' && (
                        <section className={styles.sourcePanel} aria-labelledby="file-panel-title">
                          ...
                          <div className={styles.sourcePanelBody}>
                            ...
                            <div>
                              <label className={styles.label} htmlFor="fu-files">Upload files (PDF, DOCX, XLSX, TXT, MD, JSON, XML)</label>
                              <div id="fu-files" style={dropzoneStyle} onDragOver={onDragOver} onDrop={onDrop} onClick={() => fileInputRef.current?.click()} role="button" tabIndex={0} aria-label="Click to upload or drag and drop files">
                                <div style={{ display: 'grid', placeItems: 'center', gap: 8 }}>
                                  <Icon.Upload size={24} />
                                  <div>Click to upload or drag and drop</div>
                                  <div style={{ fontSize: 12 }}>Files are uploaded to S3 immediately</div>
                                </div>
                                <input
                                  ref={fileInputRef}
                                  type="file"
                              ...
                              {uploads.length > 0 && (
                                <div style={{ marginTop: 10 }}>
                                  ...
                                  <div style={{ display: 'grid', gap: 8 }}>
                                    {uploads.map((u, i) => (
                                      <div key={`${u.name}-${i}`} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 8, alignItems: 'center', padding: '8px 10px', background: '#fff', border: '1px solid var(--line)', borderRadius: 8 }}>
                                        <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.name}</div>
                                        <div style={{ fontSize: 12 }}>
                                          {u.status === 'uploading' && <span style={{ color: 'var(--primary)' }}>Uploading…</span>}
                                          {u.status === 'done' && <span style={{ color: '#166534' }}>Uploaded</span>}
                                          {u.status === 'error' && <span style={{ color: '#9a3412' }}>Failed: {u.error}</span>}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
                  ...
                  {/* Step 3 / Step 4 blocks remain the same as your last accepted version */}
                  {activeStep === 3 && (
                    <div style={{ display: 'grid', gap: 16 }}>
                      <div style={{ background: '#eef2ff', color: '#1f2937', borderRadius: 10, padding: '14px 16px' }} aria-live="polite">
                        <strong style={{ fontWeight: 600 }}>Sources to process: </strong>{sources.length > 0 ? sources.join(', ') : 'None selected'}
                  ...
                  {activeStep === 4 && (
                    <div style={{ display: 'grid', gap: 18 }}>
                      {/* ... your existing Step 4 test & deploy UI ... */}
                      <div role="status" aria-live="polite" style={{ background: '#e9fbe7', border: '1px solid #bbf7d0', borderRadius: 8, padding: 14, color: '#14532d' }}>
                        <div style={{ fontWeight: 600, marginBottom: 6 }}>Processing complete</div>
                        <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 1.6 }}>

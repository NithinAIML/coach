...
const SelfServicePortal: React.FC = () => {
  ...
  // One-time registration gating on mount
  React.useEffect(() => {
    const cached = (() => {
      try { return JSON.parse(localStorage.getItem('coach.registration') || 'null'); } catch { return null; }
    })();
    const cachedEmail = cached?.contactEmail as string | undefined;
    if (!cachedEmail) return;

    getJSON<{registered: boolean}>(`/api/registration-status?email=${encodeURIComponent(cachedEmail)}`)
      .then(r => {
        if (r.registered) {
          setSavedStep1(true);
          setSection('dashboard'); // skip onboarding on load
        }
      })
      .catch(() => { /* non-blocking */ });
  }, []);
  ...
  const canContinue =
    activeStep === 1
      ? (isStep1Valid && savedStep1 && !savingStep1)
      : activeStep === 2
        ? !savingStep2
        : activeStep === 3
  ...
  async function submitTeamRegistration() {
    ...
    try {
      setSavingStep1(true);
      const res = await postJSON<{ok: boolean; key: string}>('/api/team-registration', form);
      if (res.ok) {
        setSavedStep1(true);
  ...
  async function handleStep2SaveThenNext() {
    ...
    let presigned: Array<{name: string; key: string; url: string; headers: Record<string,string>}> = [];

    try {
      if (files.length > 0) {
        const req = {
          contactEmail: form.contactEmail,
          files: files.map(f => ({ name: f.name, type: (f as File).type || undefined, size: f.size }))
        };
        presigned = await postJSON('/api/presign-files', req);

        // init statuses
        const status: Record<string, 'idle'|'uploading'|'done'|'error'> = {};
        presigned.forEach(p => status[p.name] = 'idle');
        ...
        for (const p of presigned) {
          ...
          if (!file) { setUploadStatuses(s => ({ ...s, [p.name]: 'error' })); throw new Error(`Missing file ${p.name}`); }
          const putRes = await fetch(p.url, { method: 'PUT', headers: p.headers, body: file });
          if (!putRes.ok) {
            setUploadStatuses(s => ({ ...s, [p.name]: 'error' }));
            throw new Error(`Upload failed: ${p.name}`);
          }
      ...
      const payload = {
        ...
        fileUpload: {
          ...fileUpload,
          filesMeta: files.map(f => ({ name: f.name, size: f.size, type: (f as File).type || undefined })),
          filesS3: presigned.map(p => ({ name: p.name, key: p.key })),
        },
      };
      const res = await postJSON<{ok: boolean; key: string}>('/api/knowledge-sources', payload);
  ...
  function onContinueClicked() {
    if (activeStep === 2) {
      if (savingStep2) return;
      handleStep2SaveThenNext();
    } else {
      goNext();
    }
  }
                ...
                <div className={styles.card}>
                  ...
                  {activeStep === 1 && (
                    <form onSubmit={(e) => e.preventDefault()} noValidate>
                      ...
                      {savedStep1 && (
                        <div role="status" aria-live="polite" style={{ background: '#e9fbe7', border: '1px solid #bbf7d0', borderRadius: 8, padding: 12, marginBottom: 12, color: '#14532d' }}>
                          Team successfully registered.
                        </div>
                      )}
                      {saveErr1 && (
                        <div role="alert" style={{ background: '#fee2e2', border: '1px solid #fecaca', borderRadius: 8, padding: 12, marginBottom: 12, color: '#7f1d1d' }}>
                          {saveErr1}
                        </div>
                      )}
                      ...
                      {/* Submit button (enables Continue after success) */}
                  ...
                  {activeStep === 2 && (
                    <>
                      {/* Error / saving banners & upload statuses */}
                      {saveErr2 && (
                        <div role="alert" style={{ background: '#fee2e2', border: '1px solid #fecaca', borderRadius: 8, padding: 12, marginBottom: 12, color: '#7f1d1d' }}>
                          {saveErr2}
                        </div>
                      ...
                      {savingStep2 && (
                        <div role="status" aria-live="polite" style={{ background: '#f3f4f6', border: '1px solid #e5e7eb', borderRadius: 8, padding: 12, marginBottom: 12, color: '#374151' }}>
                          Saving knowledge sources…
                        </div>
                      )}
                      {Object.keys(uploadStatuses).length > 0 && (
                        <div style={{ marginBottom: 10 }}>
                          {Object.entries(uploadStatuses).map(([name, st]) => (
                            <div key={name} style={{ fontSize: 12, color: st === 'error' ? '#b91c1c' : st === 'done' ? '#166534' : '#374151' }}>
                              {name}: {st}
                            </div>
                          ))}
                        </div>
                      ...
                      {selectedSource === 'Confluence' && (
                        <section className={styles.sourcePanel} aria-labelledby="confluence-panel-title">
                          ...
                          <div className={styles.sourcePanelBody}>
                            ...
                            {/* Cross-nav to File Upload */}
                            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                              <button
                                type="button"
                                className={styles.btn}
                                onClick={() => { addSource('File Upload'); setSelectedSource('File Upload'); }}
                              >
                                Go to File upload →
                              </button>
                            </div>
                      ...
                      {selectedSource === 'File Upload' && (
                        <section className={styles.sourcePanel} aria-labelledby="file-panel-title">
                          ...
                          <div className={styles.sourcePanelBody}>
                            ...
                            {/* Cross-nav to Confluence */}
                            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                              <button
                                type="button"
                                className={styles.btn}
                                onClick={() => { addSource('Confluence'); setSelectedSource('Confluence'); }}
                              >

...
const regKey = (email: string) =>
  `${PREFIX}/registration/${encodeURIComponent(email.trim().toLowerCase())}/registration.json`;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (err?.$metadata?.httpStatusCode === 404) return res.status(200).json({ registered: false });
    console.error('registration-status error:', err);
    return res.status(500).json({ error: 'status check failed' });
  }
}
